...
- `src/companion_valet/store.cljc` — `Store` protocol +
  `MemStore`: registered clients, committed records, an append-only
  audit ledger.
- `src/companion_valet/advisor.cljc` — `Advisor` protocol;
...
- `src/companion_valet/governor.cljc` —
...
- `src/companion_valet/actor.cljc` — `build-graph`, `run-request!`,
