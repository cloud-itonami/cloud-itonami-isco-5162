...
- `src/companion_valet/store.cljk` — `Store` protocol +
  `MemStore`: registered clients, committed records, an append-only
  audit ledger.
- `src/companion_valet/advisor.cljk` — `Advisor` protocol;
...
- `src/companion_valet/governor.cljk` —
...
- `src/companion_valet/actor.cljk` — `build-graph`, `run-request!`,
