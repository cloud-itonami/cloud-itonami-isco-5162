(ns companion-valet.actor
  "CompanionValetActor — the ISCO-08 5162 independent companion-and-
  valet actor as a `langgraph.graph/state-graph` (per ADR-2607011000
  / CLAUDE.md Actors section). One graph run = one assist/log
  ...
  The unconditional invariant: the CompanionAdvisor can never
  directly commit a record or dispatch a robot action the
  CompanionValetGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [companion-valet.advisor :as advisor]
            [companion-valet.governor :as governor]
            [companion-valet.store :as store]))

(defn build-graph
  "Build a compiled CompanionValetActor graph. `store` implements
  `companion-valet.store/Store`. `advisor` implements
  `companion-valet.advisor/Advisor` (defaults to `mock-advisor`).
